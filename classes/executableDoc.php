	...
	function flatSignature()
    {
        ...
        function formattedSignature()
        {
            ...
            foreach($this->_parameters as $param) {
                    ...
                    if ($classDoc) {
                            $packageDoc = $classDoc->containingPackage();
                            $signature .= '<span class="paramType"><a href="'.str_repeat('../', $myPackage->depth() + 1).$classDoc->asPath().'">'.$classDoc->name().'</a></span> <span class="paramName">$'.$param->name().'</span>, ';
                    } else {
                            $signature .= '<span class="paramType">'.$type->typeName().'</span> <span class="paramName">$'.$param->name().'</span>, ';
                    }
            }
            return '('.substr($signature, 0, -2).')';
        }
